...
{$Warnings OFF}
{$D-}
                      { these 2 lines added by Stefan Westner }
{$A+} { word align }
{$O+} { ?? ?? }
...
FUNCTION  TLZR.GetMatch       (    Source         : BufferPtr;
                                   X              : BufferIndex;
                                   SourceSize     : BufferSize;
                                   Hash           : HashTabPtr;
                               VAR Size           : WORD;
...
FUNCTION  TLZR.Compression(Source, Dest : BufferPtr;
                           SourceSize   : BufferSize) :BufferSize;
VAR
...
FUNCTION  TLZR.Decompression(Source,Dest    : BufferPtr;
                             SourceSize     : BufferSize) : BufferSize;
VAR
  X,Y,Pos                  : BufferIndex;
  Command,Size,K           : WORD;
