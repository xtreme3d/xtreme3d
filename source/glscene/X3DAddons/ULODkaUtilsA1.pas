...
uses
  Windows, SysUtils, Classes, GLVectorFileObjects, GLTexture, VectorGeometry,
  GLScene;
...
type
  ...
  TMaterialSaveType = (
    ...
    TMaterialSaveRecAEx = packed record
      Ambient,  Diffuse,
      Emission, Specular, EnvColor   : TVector;
...
type
    ...
    TLightOptA = packed record
      Enabled                      : Boolean;
      LigthStyle                   : TLightStyle;
      Position, Direction          : TAffineVector;
      Ambient, Diffuse, Specular   : TVector;
...
begin
  with aStream do begin
    ...
    if isLoadSkeleton then begin
      SMeshO := TSkeletonMeshObject.CreateOwned(aBaseMesh.MeshObjects);
      Mesh   := SMeshO;
    end else begin
      Mesh   := TMeshObject.CreateOwned(aBaseMesh.MeshObjects);
    end;
    ...
    with Mesh do begin
      ...
      aStream.Read(mMode2, SizeOf(TMeshObjectMode));
      ...
      *)
      ...
      for fg1 := 0 to kCnt -1 do begin
        ...
        with TFGVertexNormalTexIndexList.CreateOwned(FaceGroups) do begin
          aStream.Read(fMode1, SizeOf(TFaceGroupMeshMode));
          Mode := fMode1;
...
var
  ...
  ActAnim   : TActorAnimation;

  k1, firstFrame, BonesCnt,
  fr1, BoneNumb             : Integer;
  Frame1                    : TSkeletonFrame;
begin
  ...
  for fr1 := 0 to aFramesCount -1 do begin
    Frame1      := TSkeletonFrame.CreateOwned(aBaseMesh.Skeleton.Frames);
    ...
    try
      ...
      for colInd := 0 to aCollidersCount -1 do begin
        ...
        for i := 0 to 2 do begin
          aSkelColliders.Items[colInd].Scale[i]     := aSkelColliders.Items[colInd].Matrix[i, 3];
          aSkelColliders.Items[colInd].Matrix[i, 3] := 0;
        end;
      end;
      Result := True;
    except
