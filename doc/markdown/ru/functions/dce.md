...
В DCE также существует такое важное понятие, как слой (layer). Это свойство разграничивает свободу столкновений объектов (см. функцию `DceManagerSetLayers`). По умолчанию все объекты создаются на нулевом слое.
...
Задает масштаб единиц, в которых измеряются мировые расстояния в менеджере DCE. По умолчанию равен 1. Менять этот параметр не рекомендуется без веских на то причин - например, если вы создаете космосим, где масштаб геометрических вычислений задается динамически для сохранения точности вещественных чисел.
...
Задает масштаб единиц при движении (?). Это свойство пока никак движком не используется.
...
Задает логику разграничения столкновений между объектами по слоям.

- `manager` - указатель на менеджер DCE
- `ccs` - режим использования слоев:
    - `ccsDCEStandard` = 0 - приоритет слоя (режим, активный по умолчанию). Чем больше номер слоя объекта, тем он более универсален при столкновениях: объект со слоем N сталкивается со всеми объектами со слоями <= N.
    - `ccsCollisionStandard` = 1 - объекты со слоем 0 сталкиваются со всеми остальными. Остальные объекты сталкиваются друг с другом, только если их слои неодинаковы. Этот режим удобен, например, для группировки опасных для игрока объектов типа снарядов или огня.
    - `ccsHybrid` = 2 - комбинация предыдущих режимов. Объекты со слоем 0 сталкиваются со всеми остальными. Остальные объекты используют приоритет слоев, как в режиме `ccsDCEStandard`. Объекты с отрицательным номером слоя вообще не участвуют в столкновениях.
...
Задает глубину рекурсивного поиска столкновений. Чем выше это значение, тем точнее (и медленнее) работает алгоритм обнаружения столкновений для данного объекта.

- `object` - укaзатель на объект
- `depth` - глубина рекурсии. Значение по умолчанию 5.
