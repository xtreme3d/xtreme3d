...
Задает логику разграничения столкновений по слоям.

- `manager` - указатель на менеджер DCE
- `ccs` - режим использования слоев:
    - `ccsDCEStandard` = 0 - приоритет слоя. Чем больше номер слоя объекта, тем он более универсален при столкновениях: объект со слоем N сталкивается со всеми объектами со слоями <= N.
